...
# Added/modified by JLONEVAE_ANONYMOUS_AUTHORS
...
class ThreeDotsTest(absltest.TestCase):
  ...
  # "look correct"
  def x_test_data_visualize_x_y_axis(self):
    random_state = np.random.RandomState(0)
    three_dots_data = threeDots.ThreeDots()
    latent_factors_test = np.ones((7,6)) * 10 
    for i in range(1,7):
      latent_factors_test[i,i-1] += 10
  ...
  def test_data_visualize_bounds(self):
    random_state = np.random.RandomState(0)
    three_dots_data = threeDots.ThreeDots()
